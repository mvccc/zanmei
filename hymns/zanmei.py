#!/usr/bin/env python3

# vim: set fileencoding=utf-8 :

import asyncio
import re
from pathlib import Path
from urllib.parse import urlparse

import attr
from absl import app, flags, logging as log
from aiohttp import ClientSession
from bs4 import BeautifulSoup
from hanziconv import HanziConv

from hymns import fetch

# 教會聖詩 Hymns for God's People
HYMNS_INDEX_BASE_URL = "https://www.izanmei.cc/songbook/hymns-for-gods-people.html"
ZANMEI_HOMEPAGE = "https://www.izanmei.cc"
# Number of pages to fetch (527 hymns across 4 pages)
TOTAL_PAGES = 4

FLAGS = flags.FLAGS


@attr.s
class Hymn:
    name: str = attr.ib()
    no: int = attr.ib()
    url: str = attr.ib()


def _path(hymn: Hymn, download_basepath: Path | None = None) -> Path:
    if download_basepath is None:
        download_basepath = Path(FLAGS.download_basedir)
    path = download_basepath / f"{hymn.no:03d}_{hymn.name}.png"
    return path


async def index(session: ClientSession, url: str, download_basepath: Path | None = None) -> list[Hymn]:
    if download_basepath is None:
        download_basepath = Path(FLAGS.download_basedir)

    # Fetch all pages (paginated)
    all_hymns = []
    for page_num in range(1, TOTAL_PAGES + 1):
        page_url = url if page_num == 1 else f"{url}?page={page_num}"
        t = urlparse(page_url)
        index_path = download_basepath / Path(t.path).name
        if page_num > 1:
            index_path = download_basepath / f"{Path(t.path).stem}_page{page_num}.html"

        # check cache, if exists use cache
        if index_path.exists():
            log.info(f"{index_path} exists and use it as cache.")
            with index_path.open("rb") as f:
                content_bytes = f.read()
        else:
            status, content_bytes = await fetch(session, page_url)
            assert status == 200
            with index_path.open("wb") as out:
                out.write(content_bytes)

        text = content_bytes.decode()
        soup = BeautifulSoup(text, "html.parser")
        # New website structure: <li>第X首 <a href="/tab/XXX.html?songbook=1&index=X" ...>name</a>
        for li in soup.find_all("li"):
            # Look for pattern: "第X首" followed by a link
            if "第" not in li.text or "首" not in li.text:
                continue
            a = li.find("a")
            if not a or "songbook=" not in str(a.get("href", "")):
                continue

            # Extract hymn number from text like "第1首"
            no_match = re.search(r"第(\d+)首", li.text)
            if not no_match:
                continue
            no = int(no_match.group(1))

            # Skip if already in list (avoid duplicates)
            if any(h.no == no for h in all_hymns):
                continue

            # Get hymn name from link text
            name = a.text.strip()
            name = (
                HanziConv.toTraditional(name)
                .replace("傢", "家")
                .replace("山巔", "山嶺")
                .replace("嚮", "向")
                .replace("恒", "恆")
                .replace("我真希奇", "我神你是何等奇妙")
                .replace("榖中百閤花", "谷中百合花")
                .replace("渡過", "度過")
                .replace("禰", "祢")
                .replace("萬福思源", "萬福恩源")
                .replace("裏", "裡")
                .replace("贊", "讚")
                .replace("起來，宗主信徒", "齊來，宗主信徒")
                .replace("迴", "回")
                .replace("隻", "只")
                .replace("麯", "曲")
                .replace("籍我", "藉我")
                .replace("神跡", "神蹟")
            )

            # Extract the base URL without query parameters for compatibility
            href = str(a.get("href", "")).split("?")[0]
            hymn = Hymn(name=name, no=no, url=f"{ZANMEI_HOMEPAGE}{href}")
            all_hymns.append(hymn)

    return all_hymns


async def download(session: ClientSession, hymn: Hymn) -> None:
    if _path(hymn).exists():
        log.debug(f"{_path(hymn)} is already downloaded.")
        return

    log.debug(f"downloading to {_path(hymn)} ...")
    try:
        status, content = await fetch(session, hymn.url)
        assert status == 200, f"Failed to fetch hymn page: {status}"
        text = content.decode()
        soup = BeautifulSoup(text, "html.parser")

        # New website structure: <div class="img_tab"><a><img src="URL"></a></div>
        div = soup.find("div", attrs={"class": "img_tab"})
        assert div is not None, "Could not find img_tab div"

        img = div.find("img")
        assert img is not None, "Could not find img tag"

        img_url = img["src"]
        log.debug(f"Fetching image from {img_url}")

        status, content = await fetch(session, img_url)
        assert status == 200, f"Failed to fetch image: {status}"

        with _path(hymn).open("wb") as f:
            f.write(content)
    except Exception as e:
        log.exception(f"failed to download {_path(hymn)}: {e}")


def verify(path: Path, glob: str, total: int) -> None:
    path_list = list(path.glob(glob))
    downloaded = sorted(path.name.split("_", 1)[0] for path in path_list)
    expected = [f"{no:03d}" for no in range(1, total + 1)]
    missing = ["493", "494", "495"]
    log.warning(f"{missing} are missing from {HYMNS_INDEX_BASE_URL}. they are all in 492.")
    for no in missing:
        if no in expected:
            expected.remove(no)
    assert all((a == b) for a, b in zip(downloaded, expected, strict=False))
    assert len(path_list) == total


async def download_image_copy(download_basepath: Path | None = None) -> None:
    if download_basepath is None:
        download_basepath = Path(FLAGS.download_basedir)

    async with ClientSession() as session:
        hymns = await index(session, HYMNS_INDEX_BASE_URL)
        tasks = [asyncio.create_task(download(session, hymn)) for hymn in hymns]
        await asyncio.gather(*tasks)
        verify(download_basepath, "*.png", len(hymns))


if __name__ == "__main__":
    from base import initialize_logging

    flags.DEFINE_string("download_basedir", "download/zanmei", "basedir of downloaded files")

    def main(_):
        download_basepath = Path(FLAGS.download_basedir)
        download_basepath.mkdir(exist_ok=True, parents=True)
        initialize_logging()

        asyncio.run(download_image_copy())

    app.run(main)
